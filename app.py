...
@st.cache_resource(show_spinner=False)
def build_and_train_xgboost(X_train_vals, y_train_vals, random_state=42):
    """Huấn luyện XGBoost với trọng số bù cho lớp thiểu số."""
    if not XGBOOST_AVAILABLE:
        raise ImportError("Chưa cài xgboost. Cài bằng: pip install xgboost")
    y_arr = np.asarray(y_train_vals)
    n_negative = int((y_arr == 0).sum())
    n_positive = int((y_arr == 1).sum())
    class_weight = n_negative / n_positive if n_positive else 1.0
    clf = XGBClassifier(
        n_estimators=300,
        max_depth=3,
        learning_rate=0.05,
...
with st.sidebar:
    ...
    **Về ứng dụng:**
    ...
    - Sử dụng **Logistic Regression** và **XGBoost** (nếu đã cài đặt).
...
if XGBOOST_AVAILABLE:
    try:
        xgb_model = build_and_train_xgboost(X_train, y_train, random_state=int(random_seed))
    except Exception as e:
        xgb_error = str(e)
else:
    xgb_error = "Chưa cài thư viện xgboost. Cài bằng: pip install xgboost" 
...
if xgb_model is not None:
    ...
    xgb_pred_test = (xgb_prob_test >= decision_threshold).astype(int)
    xgb_cm = confusion_matrix(y_test, xgb_pred_test, labels=[0, 1])
    xgb_metrics = {
        "Accuracy": accuracy_score(y_test, xgb_pred_test),
        "Precision": precision_score(y_test, xgb_pred_test, zero_division=0),
        "Recall": recall_score(y_test, xgb_pred_test, zero_division=0),
        "F1-Score": f1_score(y_test, xgb_pred_test, zero_division=0),
        "ROC-AUC": roc_auc_score(y_test, xgb_prob_test)
    }
else:
    xgb_prob_test = xgb_pred_test = xgb_cm = xgb_metrics = None
...
st.markdown('<div class="sub-title">Ứng dụng học máy Logistic Regression và XGBoost kết hợp bộ 8 chỉ số Beneish M-Score để phát hiện rủi ro thao túng số liệu kế toán</div>', unsafe_allow_html=True)
...
with tab_model:
    ...
    st.markdown("#### ⚖️ So sánh Logistic Regression và XGBoost")
    if xgb_model is not None:
        comparison_df = pd.DataFrame([
            {"Mô hình": "Logistic Regression", "Accuracy": acc, "Precision": prec, "Recall": rec,
             "F1-Score": f1, "ROC-AUC": roc_auc_score(y_test, y_prob_test)},
            {"Mô hình": "XGBoost", **xgb_metrics}
        ])
        st.dataframe(
            comparison_df.style.format({c: "{:.3f}" for c in
                ["Accuracy", "Precision", "Recall", "F1-Score", "ROC-AUC"]}),
            use_container_width=True
        )
        st.markdown("##### Ma trận nhầm lẫn — XGBoost")
        st.dataframe(
            pd.DataFrame(xgb_cm, index=["Thực tế 0", "Thực tế 1"],
                         columns=["Dự báo 0", "Dự báo 1"]),
            use_container_width=True
        )
    else:
        st.warning(f"Không thể chạy XGBoost: {xgb_error}")
...
with tab_predict:
    ...
    with subtab_single:
        ...
        if st.button("🚀 Thực hiện Dự báo Nguy cơ", type="primary", use_container_width=True):
            ...
            if xgb_model is not None:
                xgb_single_prob = float(xgb_model.predict_proba(input_df)[0, 1])
                st.markdown("##### 3. Dự báo bổ sung bằng XGBoost")
                st.progress(xgb_single_prob)
                st.markdown(f"**Xác suất gian lận ước tính (XGBoost):** {xgb_single_prob*100:.2f}%")
                if xgb_single_prob >= decision_threshold:
                    st.warning("XGBoost phân loại: CẢNH BÁO NGUY CƠ")
                else:
                    st.success("XGBoost phân loại: Dưới ngưỡng cảnh báo")
        ...
        if batch_file is not None:
            try:
                ...
                if missing_in_batch:
                    ...
                    if xgb_model is not None:
                        xgb_batch_probs = xgb_model.predict_proba(X_batch)[:, 1]
                        xgb_batch_preds = (xgb_batch_probs >= decision_threshold).astype(int)
                        result_df["XGBoost - Xác suất Gian lận (%)"] = np.round(xgb_batch_probs * 100, 2)
                        result_df["XGBoost - Dự báo"] = [
                            "CẢNH BÁO GIAN LẬN" if p == 1 else "AN TOÀN"
                            for p in xgb_batch_preds
                        ]
